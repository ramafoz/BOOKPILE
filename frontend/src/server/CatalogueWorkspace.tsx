...
export function BookDetails({ libraryId, book, location, reading, perspectiveName, reviews, loans, locale, onClose, onEdit }: {
  libraryId: string;
  book: ServerBook;
  ...
  const detailEnumLabels: Record<string, string> = {
    UNKNOWN: copy("unknown"), ORIGINAL: copy("original"), TRANSLATED: copy("translated"),
    FICTION: copy("fiction"), NON_FICTION: copy("nonFiction"), HARDCOVER: copy("hardcover"),
    PAPERBACK: copy("paperback"), FLEXIBOUND: copy("flexibound"), SPIRAL: copy("spiral"),
    STAPLED: copy("stapled"), OTHER: copy("other"), CONVENTIONAL_BOOK: copy("conventionalBook"),
    COMIC_GRAPHIC_NOVEL: copy("comicGraphicNovel"), ATLAS: copy("atlas"), REFERENCE: copy("reference"),
    ART_PHOTOGRAPHY_ILLUSTRATED: copy("artPhotographyIllustrated"), MAGAZINE_PERIODICAL: copy("magazinePeriodical"),
  };
  const rows: Array<[string, string | number | boolean | null]> = [
    [copy("subtitle"), book.subtitle], [copy("isbn10"), book.isbn_10], [copy("isbn13"), book.isbn_13],
    [copy("pages"), book.page_count], [copy("publisher"), book.publisher],
    [copy("currentEditionYear"), book.current_ed_year], [copy("originalPublicationYear"), book.original_publication_year],
    [copy("language"), book.language], [copy("originalLanguage"), book.original_language],
    [copy("translation"), book.translation_status], [copy("edition"), book.edition_number],
    [copy("category"), book.fiction_category], [copy("binding"), book.binding],
    [copy("publicationType"), book.publication_type], [copy("genres"), book.genre_text],
    [copy("series"), book.series_name], [copy("seriesVolume"), book.series_volume],
    [copy("acquired"), book.is_original_collection ? copy("originalCollection") : book.acquisition_date],
    [copy("height"), book.height_mm ? `${book.height_mm} mm` : null],
    [copy("width"), book.width_mm ? `${book.width_mm} mm` : null],
    [copy("thickness"), book.thickness_mm ? `${book.thickness_mm} mm` : null],
    [copy("notes"), book.notes],
    [copy("physicalLocation"), location ?? null],
  ];
  return <div className="server-modal-backdrop"><section className="server-catalogue-dialog details" role="dialog" aria-modal="true">
    ...
    <p className="server-card-eyebrow">{copy("readOnlyRecord")}</p>
    <div className="server-book-details-heading"><CoverImage libraryId={libraryId} book={book} copy={copy} /><div><h2>{book.title}</h2><p className="server-book-author">{book.display_author}</p></div></div>
    {!!book.contributors.length && <section><h3>{copy("contributors")}</h3><div className="server-contributor-credits">{book.contributors.map((item) => <span key={item.id}><b>{item.role_label}</b>{item.name}</span>)}</div></section>}
    {reading && <ReadingSummary reading={reading} perspectiveName={perspectiveName ?? copy("selectedOwner")} locale={locale} />}
    <LoanSummary loans={loans ?? null} locale={locale} />
    <GoodreadsSummary reviews={reviews ?? []} locale={locale} />
    <dl className="server-book-metadata">{rows.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value === null || value === "" ? copy("notRecorded") : detailEnumLabels[String(value)] ?? String(value).replaceAll("_", " ")}</dd></div>)}</dl>
    <div className="server-dialog-actions"><button type="button" onClick={onClose}>{copy("close")}</button>{onEdit && <button className="confirm" type="button" onClick={onEdit}><Pencil size={16} /> {copy("editBook")}</button>}</div>
  </section></div>;
}
...
function CandidateReview({ candidate, copy, onApply }: {
  ...
  const allValues: Array<[CandidateMetadataKey, string, string | number | null]> = [
    ["title", copy("title"), candidate.title], ["author", copy("authors"), candidate.authors.join(" · ")],
    ["isbn_10", copy("isbn10"), candidate.identifiers.isbn_10], ["isbn_13", copy("isbn13"), candidate.identifiers.isbn_13],
    ["subtitle", copy("subtitle"), candidate.subtitle], ["page_count", copy("pages"), candidate.page_count],
    ["publisher", copy("publisher"), candidate.publisher], ["current_ed_year", copy("editionYear"), candidate.current_ed_year],
    ["original_publication_year", copy("originalYear"), candidate.original_publication_year], ["language", copy("language"), candidate.language],
    ["edition_number", copy("edition"), candidate.edition_number], ["fiction_category", copy("category"), candidate.fiction_category],
    ["binding", copy("binding"), candidate.binding], ["publication_type", copy("publicationType"), candidate.publication_type],
    ["genre_text", copy("genres"), candidate.genre_text], ["series_name", copy("series"), candidate.series_name], ["series_volume", copy("seriesVolume"), candidate.series_volume],
  ];
  ...
  return <div className="server-candidate-review">
    <p><b>{copy("chooseTransferValues")}</b><small>{copy("transferNoneAutomatic")}</small></p>
    ...
    <button type="button" disabled={!selected.size} onClick={() => onApply(selected)}>{copy("applyMetadata")}</button>
  </div>;
}

function BookEditor({ libraryId, initial, initialCover, roles, options, physical, bookId, initialPlacement, batchMode, scanFlow, batchAddedCount = 0, locale, onClose, onOpenExisting, onSave }: {
  libraryId: string;
  ...
  const copy = catalogueCopy(locale);
  const editorEnumLabels: Record<string, string> = {
    ...
    ATLAS: copy("atlas"), REFERENCE: copy("reference"), ART_PHOTOGRAPHY_ILLUSTRATED: copy("artPhotographyIllustrated"),
  ...
  async function lookUpIsbn(value = isbnInput) {
    if (!value.trim()) { setIsbnError(copy("enterIsbn")); return; }
  ...
  async function submit(event: FormEvent) {
    event.preventDefault(); setBusy(true); setError(""); setProgress(copy("saveBook"));
    try {
      if (creatingAtOpen && loan.enabled && personal.readingMode === "ACTIVE") throw new Error(copy("loanReadingConflict"));
  ...
  return <div className="server-modal-backdrop"><section className="server-catalogue-dialog editor" role="dialog" aria-modal="true">
    <button className="server-dialog-close" type="button" onClick={onClose} aria-label={copy("close")} disabled={busy}><X /></button>
    <p className="server-card-eyebrow">{batchMode ? copy("batchCataloguing") : copy("sharedRecord")}</p><h2>{initial.title ? copy("editBook") : batchMode ? copy("addNextBook") : copy("addBook")}</h2>
    {batchMode && <p className="server-field-help"><b>{copy(batchAddedCount === 1 ? "batchCountOne" : "batchCountMany", { count: batchAddedCount })}</b> {copy("batchHelp")}</p>}
    {error && <div className="server-message error">{error}</div>}
    <form onSubmit={submit} className="server-book-form">
      <fieldset className={`server-isbn-identify${scanFlow ? " scan-flow" : ""}`}><legend>{copy("identifyIsbn")}</legend>
        <p className="server-field-help">{copy("barcodePrivacy")}</p>
        <div className="server-isbn-actions"><label>ISBN-10 or ISBN-13<input value={isbnInput} maxLength={40} inputMode="text" placeholder="978-…" onChange={(event) => { setIsbnInput(event.target.value); setIsbnLookup(null); setIsbnError(""); }} /></label>
          <button type="button" disabled={isbnBusy || barcodeBusy} onClick={() => void lookUpIsbn()}><ScanBarcode size={17} /> {isbnBusy ? copy("lookingUp") : copy("lookupIsbn")}</button>
          <label className={`server-barcode-photo${barcodeBusy ? " busy" : ""}`}><Camera size={17} /> {barcodeBusy ? copy("readingPhoto") : copy("takeBarcodePhoto")}<input ref={barcodeInput} type="file" accept="image/jpeg,image/png,image/webp" capture="environment" disabled={isbnBusy || barcodeBusy} onChange={(event) => void decodeBarcode(event.target.files?.[0] ?? null)} /></label>
        </div>
        {(isbnBusy || barcodeBusy) && <div className="server-isbn-progress" role="status"><LoaderCircle size={18} /> {barcodeBusy ? copy("barcodeReadingLocally") : copy("checkingSources")}</div>}
        {isbnError && <div className="server-message error">{isbnError}</div>}
        {isbnLookup?.catalogue_matches.length ? <div className="server-isbn-matches"><b>{copy("isbnAlreadyPresent")}</b>{isbnLookup.catalogue_matches.map((match) => <button type="button" key={match.book_id} onClick={() => onOpenExisting(match.book_id)}>{match.title} · {match.author}</button>)}</div> : null}
        {isbnLookup && !isbnLookup.candidates.length && !isbnLookup.catalogue_matches.length ? <div className="server-message">{copy("noBibliographicRecord")}</div> : null}
        {isbnLookup?.candidates.map((candidate, index) => <article className="server-isbn-candidate" key={`${candidate.source}-${candidate.source_record_id ?? index}`}><header><small>{candidate.source.replaceAll("_", " ")}</small><b>{candidate.title}</b><span>{candidate.authors.join(" · ")}</span></header><CandidateReview candidate={candidate} copy={copy} onApply={(selected) => applyCandidate(candidate, selected)} /></article>)}
      </fieldset>
      <fieldset><legend>{copy("requiredInformation")}</legend><div className="server-form-grid">
        <label>{copy("titleRequired")}<input required maxLength={500} value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /></label>
        <label>{copy("authorDisplayRequired")}<input required maxLength={500} value={draft.author} onChange={(event) => setDraft({ ...draft, author: event.target.value })} readOnly={draft.contributors.filter((item) => item.role_code === "AUTHOR").length >= 2} /></label>
      </div></fieldset>
      <fieldset><legend>{copy("privateCover")}</legend>
        <p className="server-field-help">{copy("coverPrivacy")}</p>
        <div className="server-cover-editor">
          {coverPreview && !removeCover
            ? <img src={coverPreview} alt={copy("selectedCoverPreview")} />
            : initialCover && !removeCover
              ? <span className="server-cover-existing"><BookOpen size={28} /> {copy("storedCover")}</span>
              : <span className="server-cover-empty"><BookOpen size={28} /> {copy("noCover")}</span>}
          <div><label className="server-cover-picker">{copy("chooseCover")}<input type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif,.heic,.heif" onChange={(event) => { setCoverFile(event.target.files?.[0] ?? null); setRemoveCover(false); }} /></label>
          {(initialCover || coverFile) && !removeCover && <button type="button" className="server-cover-remove" onClick={() => { setCoverFile(null); setRemoveCover(true); }}>{copy("removeCover")}</button>}
          {removeCover && initialCover && <button type="button" onClick={() => setRemoveCover(false)}>{copy("keepCover")}</button>}</div>
        </div>
      </fieldset>
      <fieldset><legend>{copy("contributors")}</legend><p className="server-field-help">{copy("contributorHelp")}</p>
        <div className="server-contributor-editor">{draft.contributors.map((item, index) => <div key={`${index}-${item.role_code}`}>
          ...
          <input required maxLength={300} value={item.name} onChange={(event) => setContributors(draft.contributors.map((entry, position) => position === index ? { ...entry, name: event.target.value } : entry))} placeholder={copy("contributorName")} />
          <button type="button" aria-label={copy("moveUp")} disabled={index === 0} onClick={() => { const next = [...draft.contributors]; [next[index - 1], next[index]] = [next[index], next[index - 1]]; setContributors(next); }}><ArrowUp size={15} /></button>
          <button type="button" aria-label={copy("moveDown")} disabled={index === draft.contributors.length - 1} onClick={() => { const next = [...draft.contributors]; [next[index], next[index + 1]] = [next[index + 1], next[index]]; setContributors(next); }}><ArrowDown size={15} /></button>
          <button type="button" aria-label={copy("removeContributor")} onClick={() => setContributors(draft.contributors.filter((_, position) => position !== index))}><Trash2 size={15} /></button>
        </div>)}</div>
        <button type="button" onClick={() => setContributors([...draft.contributors, { role_code: roles[0]?.code ?? "AUTHOR", name: "" }])}><Plus size={15} /> {copy("addContributor")}</button>
      </fieldset>
      <fieldset><legend>{copy("editionClassification")}</legend><div className="server-form-grid">
        <label>{copy("subtitle")}<input value={draft.subtitle ?? ""} onChange={(e) => text("subtitle", e.target.value)} /></label>
        <label>{copy("pages")}<input type="number" min="1" value={draft.page_count ?? ""} onChange={(e) => setDraft({ ...draft, page_count: numberValue(e.target.value) })} /></label>
        <label>{copy("publisher")}<input list="server-publishers" value={draft.publisher ?? ""} onChange={(e) => text("publisher", e.target.value)} /></label>
        <label>{copy("currentEditionYear")}<input type="number" min="1000" max="9999" value={draft.current_ed_year ?? ""} onChange={(e) => setDraft({ ...draft, current_ed_year: numberValue(e.target.value) })} /></label>
        <label>{copy("originalPublicationYear")}<input type="number" min="1000" max="9999" value={draft.original_publication_year ?? ""} onChange={(e) => setDraft({ ...draft, original_publication_year: numberValue(e.target.value) })} /></label>
        <label>{copy("editionNumber")}<input type="number" min="1" value={draft.edition_number ?? ""} onChange={(e) => setDraft({ ...draft, edition_number: numberValue(e.target.value) })} /></label>
        <label>{copy("category")}<select value={draft.fiction_category ?? ""} onChange={(e) => setDraft({ ...draft, fiction_category: e.target.value || null })}><option value="">{copy("notRecorded")}</option><option value="FICTION">{copy("fiction")}</option><option value="NON_FICTION">{copy("nonFiction")}</option></select></label>
        <label>{copy("genres")}<input list="server-genres" value={draft.genre_text ?? ""} onChange={(e) => text("genre_text", e.target.value)} placeholder={copy("commaSeparated")} /></label>
        <label>{copy("series")}<input list="server-series" value={draft.series_name ?? ""} onChange={(e) => text("series_name", e.target.value)} /></label>
        <label>{copy("seriesVolume")}<input value={draft.series_volume ?? ""} onChange={(e) => text("series_volume", e.target.value)} /></label>
      </div></fieldset>
      <fieldset><legend>{copy("languageTranslation")}</legend><div className="server-form-grid">
        <label>{copy("language")}<input list="server-languages" value={draft.language ?? ""} onChange={(e) => text("language", e.target.value)} /></label>
        <label>{copy("originalLanguage")}<input list="server-original-languages" value={draft.original_language ?? ""} onChange={(e) => text("original_language", e.target.value)} /></label>
        <label>{copy("translationStatus")}<select value={draft.translation_status} onChange={(e) => setDraft({ ...draft, translation_status: e.target.value as ServerBookWrite["translation_status"] })}><option value="UNKNOWN">{copy("unknown")}</option><option value="ORIGINAL">{copy("originalLanguageValue")}</option><option value="TRANSLATED">{copy("translated")}</option></select></label>
      </div></fieldset>
      <fieldset><legend>{copy("identifiers")}</legend><div className="server-form-grid">
        <label>{copy("isbn10")}<input value={draft.isbn_10 ?? ""} onChange={(e) => text("isbn_10", e.target.value)} /></label><label>{copy("isbn13")}<input value={draft.isbn_13 ?? ""} onChange={(e) => text("isbn_13", e.target.value)} /></label>
      </div></fieldset>
      <fieldset><legend>{copy("copyDetails")}</legend><div className="server-form-grid">
        <label>{copy("binding")}<select value={draft.binding ?? ""} onChange={(e) => setDraft({ ...draft, binding: e.target.value || null })}><option value="">{copy("notRecorded")}</option>{["HARDCOVER", "PAPERBACK", "FLEXIBOUND", "SPIRAL", "STAPLED", "OTHER"].map((value) => <option key={value} value={value}>{editorEnumLabels[value]}</option>)}</select></label>
        <label>{copy("publicationType")}<select value={draft.publication_type ?? ""} onChange={(e) => setDraft({ ...draft, publication_type: e.target.value || null })}><option value="">{copy("notRecorded")}</option>{["CONVENTIONAL_BOOK", "COMIC_GRAPHIC_NOVEL", "ATLAS", "REFERENCE", "ART_PHOTOGRAPHY_ILLUSTRATED", "MAGAZINE_PERIODICAL", "OTHER"].map((value) => <option key={value} value={value}>{editorEnumLabels[value]}</option>)}</select></label>
        <label>{copy("acquisitionDate")}<input type="date" value={draft.acquisition_date ?? ""} disabled={draft.is_original_collection} onChange={(e) => text("acquisition_date", e.target.value)} /></label>
        <label className="server-inline-check"><input type="checkbox" checked={draft.is_original_collection} onChange={(e) => setDraft({ ...draft, is_original_collection: e.target.checked, acquisition_date: e.target.checked ? null : draft.acquisition_date })} /> {copy("originalCollection")}</label>
        <label>{copy("height")} (mm)<input type="number" min="1" max="10000" value={draft.height_mm ?? ""} onChange={(e) => setDraft({ ...draft, height_mm: numberValue(e.target.value) })} /></label><label>{copy("width")} (mm)<input type="number" min="1" max="10000" value={draft.width_mm ?? ""} onChange={(e) => setDraft({ ...draft, width_mm: numberValue(e.target.value) })} /></label><label>{copy("thickness")} (mm)<input type="number" min="1" max="10000" value={draft.thickness_mm ?? ""} onChange={(e) => setDraft({ ...draft, thickness_mm: numberValue(e.target.value) })} /></label>
      </div></fieldset>
      {physical && <fieldset><legend>{copy("physicalLocation")}</legend><p className="server-field-help">{copy("physicalLocationHelp")}</p><div className="server-form-grid">
        <label>{copy("bookcase")}<select value={bookcaseId} onChange={(event) => { const value = event.target.value; setBookcaseId(value); const firstShelf = physical.bookcases.find((item) => item.id === value)?.shelves[0]; const firstContainer = firstShelf?.containers[0]; setShelfId(firstShelf?.id ?? ""); setContainerId(firstContainer?.id ?? ""); setPosition(firstContainer ? String(firstContainer.book_count + 1) : ""); }}><option value="">{copy("noPhysicalLocation")}</option>{physical.bookcases.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label>{copy("shelf")}<select disabled={!bookcaseId} value={shelfId} onChange={(event) => { const value = event.target.value; setShelfId(value); const first = shelves.find((item) => item.id === value)?.containers[0]; setContainerId(first?.id ?? ""); setPosition(first ? String(first.book_count + 1) : ""); }}><option value="">{copy("chooseShelf")}</option>{shelves.map((item) => <option key={item.id} value={item.id}>{copy("shelf")} {item.shelf_number}</option>)}</select></label>
        <label>{copy("container")}<select disabled={!shelfId} value={containerId} onChange={(event) => { const value = event.target.value; setContainerId(value); const selectedContainer = containers.find((item) => item.id === value); setPosition(selectedContainer ? String(selectedContainer.book_count + (selectedContainer.id === placed?.container_id ? 0 : 1)) : ""); }}><option value="">{copy("chooseContainer")}</option>{containers.map((item) => <option key={item.id} value={item.id}>{item.layer === "BACKGROUND" ? copy("background") : copy("foreground")} {item.container_type === "ROW" ? copy("row") : copy("pile")} {item.container_number}</option>)}</select></label>
        <label>{copy("position")}<input type="number" min="1" required={Boolean(containerId)} disabled={!containerId} value={position} onChange={(event) => setPosition(event.target.value)} /></label>
      </div></fieldset>}
      <fieldset><legend>{copy("libraryNotes")}</legend><div className="server-form-grid">
        <label className="wide">{copy("notes")}<textarea rows={4} maxLength={4000} value={draft.notes ?? ""} onChange={(e) => text("notes", e.target.value)} /></label>
      </div></fieldset>
      {creatingAtOpen && <fieldset className="server-initial-reading"><legend>{copy("myReading")} <small>{copy("optional")}</small></legend>
        <p className="server-field-help">{copy("initialReadingHelp")}</p>
        <div className="server-form-grid"><label className="wide">{copy("readingRecord")}<select value={personal.readingMode} onChange={(event) => setPersonal({ ...personal, readingMode: event.target.value as InitialPersonalReadingWrite["readingMode"], datesUnknown: false, startedDate: "", finishedDate: "" })}><option value="NONE">{copy("noReadingRecord")}</option><option value="HISTORICAL">{copy("historicalCompleted")}</option><option value="ACTIVE">{copy("activeStarted")}</option></select></label></div>
        {personal.readingMode !== "NONE" && <div className="server-form-grid server-initial-reading-dates">
          {personal.readingMode === "HISTORICAL" && <label className="server-compact-check wide"><input type="checkbox" checked={personal.datesUnknown} onChange={(event) => setPersonal({ ...personal, datesUnknown: event.target.checked, startedDate: event.target.checked ? "" : personal.startedDate, finishedDate: event.target.checked ? "" : personal.finishedDate })} /> {copy("readingDatesUnknown")}</label>}
          {!personal.datesUnknown && <><label>{copy("started")}<input type="date" required value={personal.startedDate} onChange={(event) => setPersonal({ ...personal, startedDate: event.target.value })} /></label>{personal.readingMode === "HISTORICAL" && <label>{copy("finished")}<input type="date" required value={personal.finishedDate} onChange={(event) => setPersonal({ ...personal, finishedDate: event.target.value })} /></label>}</>}
        </div>}
        <div className="server-form-grid"><label className="wide">{copy("goodreadsUrl")}<input type="url" placeholder="https://www.goodreads.com/review/show/..." value={personal.goodreadsUrl} onChange={(event) => setPersonal({ ...personal, goodreadsUrl: event.target.value })} /></label></div>
      </fieldset>}
      {creatingAtOpen && <fieldset className="server-initial-loan"><legend>{copy("currentLoan")} <small>{copy("optional")}</small></legend>
        <label className="server-compact-check"><input type="checkbox" checked={loan.enabled} onChange={(event) => setLoan({ ...EMPTY_INITIAL_LOAN, enabled: event.target.checked })} /> {copy("copyCurrentlyLoaned")}</label>
        {loan.enabled && <><p className="server-field-help">{copy("retainedPositionHelp")}</p><div className="server-form-grid">
          <label>{copy("loanedToRequired")}<input required maxLength={300} value={loan.loaned_to} onChange={(event) => setLoan({ ...loan, loaned_to: event.target.value })} /></label>
          <label>{copy("loanDateLabel")} <small>{copy("optionalUnknown")}</small><input type="date" value={loan.loaned_date ?? ""} onChange={(event) => setLoan({ ...loan, loaned_date: event.target.value || null })} /></label>
          <label>{copy("expectedReturnLabel")} <small>{copy("optional")}</small><input type="date" value={loan.expected_return_date ?? ""} onChange={(event) => setLoan({ ...loan, expected_return_date: event.target.value || null })} /></label>
          <label className="wide">{copy("privateOwnerNotes")}<textarea rows={2} maxLength={4000} value={loan.notes ?? ""} onChange={(event) => setLoan({ ...loan, notes: event.target.value || null })} /></label>
        </div></>}
      </fieldset>}
      ...
      {progress && <div className="server-save-progress" role="status">{progress}<small>{progress === copy("processingCover") ? copy("processingPhotoHelp") : ""}</small></div>}
      <div className="server-dialog-actions"><button type="button" onClick={onClose} disabled={busy}>{batchMode ? copy("finishBatch") : copy("cancel")}</button><button className="confirm" disabled={busy} type="submit">{busy ? copy("pleaseWait") : batchMode ? scanFlow ? copy("saveScanNext") : copy("saveAddNext") : copy("saveBook")}</button></div>
    </form>
  </section></div>;
}
...
export default function CatalogueWorkspace({ library, memberSummary, signedInUserId, perspectives, locale, onOpenProfile, onSetUpMap }: {
  ...
  async function save(book: ServerBookWrite, coverFile: File | null, removeCover: boolean, placement: PlacementWrite | null, personal: InitialPersonalReadingWrite | null, loan: InitialLoanWrite | null, reportProgress: (message: string) => void) {
    ...
    try {
      ...
      if (personal?.readingMode === "HISTORICAL") {
        reportProgress(copy("savingReadingHistory"));
        ...
        reportProgress(copy("startingReading"));
        await serverApi.startReading(library.library_id, saved.id, personal.startedDate);
      }
    } catch (caught) {
      if (wasNew) throw new Error(copy("partialSave", { error: message(caught, copy("serverUnavailable")) }));
      throw caught;
    }
    if (batchMode) {
      ...
      setEditing({ id: null, book: { ...EMPTY_BOOK, contributors: [] }, cover: null });
      pushNotice(copy("savedNext", { title: saved.title }));
    } else {
      setEditing(null);
      setBatchMode(false);
      setBatchPlacement(null);
      pushNotice(wasNew ? copy("bookSaved") : copy("bookUpdated"));
    }
    await Promise.all([load(query), serverApi.catalogueOptions(library.library_id).then(setOptions)]);
  }
  async function remove(book: ServerBookSummary) { if (!window.confirm(copy("deleteBookConfirm", { title: book.title }))) return; try { await serverApi.deleteBook(library.library_id, book.id, book.title); pushNotice(copy("bookDeleted")); await load(query); if (library.can_view_map) setPhysical(await serverApi.physicalLibrary(library.library_id)); } catch (caught) { setError(message(caught, copy("serverUnavailable"))); } }
  ...
  return <section className="server-catalogue-workspace">
    ...
    {details && <BookDetails libraryId={library.library_id} book={details} location={locationLabel(physical, details.id, copy)} reading={detailsReading} perspectiveName={selectedPerspective?.username} reviews={detailsReviews} loans={detailsLoans} locale={locale} onClose={() => setDetails(null)} onEdit={library.role === "OWNER" ? () => void edit(details.id) : null} />}{editing && <BookEditor key={editorSequence} libraryId={library.library_id} initial={editing.book} initialCover={editing.cover} roles={options.contributor_roles} options={options} physical={physical} bookId={editing.id} initialPlacement={batchMode ? batchPlacement : null} batchMode={batchMode} scanFlow={scanFlow} batchAddedCount={batchAddedCount} locale={locale} onOpenExisting={(bookId) => { setEditing(null); setBatchMode(false); setScanFlow(false); void openDetails(bookId); }} onClose={() => { setEditing(null); setBatchMode(false); setScanFlow(false); setBatchPlacement(null); setBatchAddedCount(0); }} onSave={save} />}
    {readingAction && <ReadingActionDialog libraryId={library.library_id} book={readingAction.book} reading={readingAction.reading} locale={locale} onClose={() => setReadingAction(null)} onChanged={refreshPersonalData} />}
    {readingManager && <ReadingManager libraryId={library.library_id} book={readingManager} locale={locale} onClose={() => setReadingManager(null)} onChanged={refreshPersonalData} />}
    {loanManager && <LoanManager libraryId={library.library_id} book={loanManager} locale={locale} onClose={() => setLoanManager(null)} onChanged={refreshLoanData} />}
  </section>;
}
