...
Status: shortlist prepared on 2026-09-11; no service has been purchased and no
provider has passed BOOKPILE acceptance yet. Published prices are comparison
inputs, not contractual quotes, and must be checked again immediately before
purchase.
...
| Domain and DNS | undecided | This does not change the application adapter. Keeping registration independent from the VPS reduces recovery friction; Dinahosting is a reasonable support-oriented candidate but needs a concrete domain quote. |
...
- Whether the services will be contracted as an individual or a professional/
  organization.
- The acceptable recurring monthly budget before VAT, excluding a domain.
- Whether encrypted operational backups and email metadata may reside elsewhere
  in the EU while the VPS and active user objects remain in Spain.
- The desired domain, or whether a temporary staging subdomain already exists.
- The private alert destination for failed systemd units (email alone, or an
  additional operator channel).
